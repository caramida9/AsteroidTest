...
void FontEngine::DestroyFontEngine(FontEngine *engine)
{
	...
	for (FontTypeMap::iterator fontIt = engine->fonts_.begin();
		fontIt != engine->fonts_.end();
		++fontIt)
	{
		DestroyFont(&(fontIt->second));
	}

	engine->textureSampler_->Release();
...
void FontEngine::DestroyFont(Font *font)
{
